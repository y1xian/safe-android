...
public class YToast {
    ...
    private static Context context= YFrame.getContext();
    ...
    public static Toast warning( @NonNull String message, int duration, boolean withIcon) {
        Drawable icon=null;
        if (withIcon){
            icon= YOutdatedUtils.getDrawable( R.drawable.yyx_toast_warning);
        }
    ...
    public static Toast info( @NonNull String message, int duration, boolean withIcon) {
        Drawable icon=null;
        if (withIcon){
            icon= YOutdatedUtils.getDrawable( R.drawable.yyx_toast_info);
        }
    ...
    public static Toast success( @NonNull String message, int duration, boolean withIcon) {
        Drawable icon=null;
        if (withIcon){
            icon= YOutdatedUtils.getDrawable( R.drawable.yyx_toast_success);
        }
    ...
    public static Toast error( @NonNull String message, int duration, boolean withIcon) {
        Drawable icon=null;
        if (withIcon){
            icon= YOutdatedUtils.getDrawable( R.drawable.yyx_toast_error);
        }
    ...
    public static Toast custom( @NonNull String message, @DrawableRes int iconRes,
        ...
        return custom( message, YOutdatedUtils.getDrawable( iconRes), textColor,tintColor, duration);
    }
    ...
    public static Toast custom( @NonNull String message, Drawable icon,
                                            @ColorInt int textColor, @ColorInt int tintColor, int duration) {
        View toastLayout = LayoutInflater.from(context).inflate(R.layout.yyx_toast_view, null);
        ...
        Drawable drawableFrame= YOutdatedUtils.getDrawable(R.drawable.yyx_toast_frame);
        drawableFrame.setColorFilter(new PorterDuffColorFilter(tintColor, PorterDuff.Mode.SRC_IN));
        YOutdatedUtils.setBackground(toastLayout, drawableFrame);

        if (icon == null){
            toastIcon.setVisibility(View.GONE);
        }else{
            YOutdatedUtils.setBackground(toastIcon, icon);
        }
