 ...
 */
public class YBrightnessUtils {

    private YBrightnessUtils() {
    ...
    public static boolean isAutoBrightnessEnabled() {
        try {
            int mode = Settings.System.getInt(
                    YFrame.getContext().getContentResolver(),
    ...
    public static boolean setAutoBrightnessEnabled(final boolean enabled) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M
                && !Settings.System.canWrite(YFrame.getContext())) {
            Intent intent = new Intent(android.provider.Settings.ACTION_MANAGE_WRITE_SETTINGS);
            intent.setData(Uri.parse("package:" + YFrame.getContext().getPackageName()));
            intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
            YFrame.getContext().startActivity(intent);
            return false;
        }
    ...
    public static boolean setBrightness(@IntRange(from = 0, to = 255) final int brightness) {
        ...
        ContentResolver resolver = YFrame.getContext().getContentResolver();
